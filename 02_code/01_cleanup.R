# ------------------------------------------------------------------------------
# Title: Code for cleanup and extension of raw data
...
########## UCR DATA###############
ucr_kaplan<-read_dta("03_raw/UCR_Kaplan.dta")
...
leoka<-read_dta("03_raw/LEOKA.dta")
...
#Reading csvs
temp=list.files(path="03_raw",pattern="emp.csv", full.names = T)
...
base_asg<-read_dta("03_raw/ASG_Original.dta")
...
crosswalk<-read_dta("03_raw/Crosswalk.dta")
...
temp_pop=list.files(path="03_raw",pattern="*.txt", full.names=T)
...
#Dropping all empty observations and Infinite obs
final_panel<-final_panel%>%
  drop_na()%>%
...
final_panel<-final_panel%>%rename(Y1=actual_murder,
                                  ...
                                  C1=population)
#Creating hybrid dataset for wide

new<-final_panel%>%
  filter(year>2010)%>%
  select(-cityid,-GOVID, STATE,ORI7,year,stateyear, Y1:S,Z,C1:W)

hybrid<-base%>%
  select(-cityid,-numcities)%>%
  rbind(new)%>%arrange(ORI7,year)

#Outputting final dataset as csv

write.csv(hybrid, "04_output/wide.csv")

# Outputting novel dataset as csv


write.csv(final_panel, "04_output/novel.csv")
