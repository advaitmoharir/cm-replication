...
replicate<-function(x){
  ...
  # Y6-theft, Y7-motor vehical theft, Y8-violent crimes,
  ...
  colnames(GMM)<-c("estimate", "standard error")
  results$GMM<-GMM # attachong results of GMM to the list
  ...
  for (i in 1:(length(crimes)-1)) {
    for (j in (i+1):length(crimes)) {
      if(((crimes[i]=="Murder" | crimes[i]=="Rape" | crimes[i]=="Robbery" |
           crimes[i]=="Assault") & crimes[j]=="Sum of violent crimes")==TRUE){break}
      if(((crimes[i]=="Burglary" | crimes[i]=="Larceny" | 
           crimes[i]=="Motor vehicle theft") & crimes[j]=="Sum of property crimes")==TRUE){break}
      crime.pairs <- c(crimes[i],crimes[j])
      X1<-rYw[,crime.pairs]
      X2<-cbind(rSw,rZw)
      X<-cbind(X1,X2)
      gmm.est<-gmm(g=ghat2,x=X,t0=rep(0.1,ncol(X1)),method="L-BFGS-B",vcov = "iid",type = "twoStep")
      ...
      out0<-c(crime.pairs,p.values)
      out<-rbind(out,out0)
    }
  }
  colnames(out)<-c("crime 1","crime 2","p value")
  return(out)
